...
Undo has always been problematic for me in Emacs. I'm trying to wean myself off of ~undo-tree-mode~ due to its proclivity for losing my history. This binding seems to improve the aesthetics of undo/redo, though it's certainly not perfect.
...
The Emacs dashboard looks okay once you disable its cutesy messages. Unfortunately, since I haven't yet figured out how to make macOS open Emacs on another screen, which makes the visual effect of seeing a dashboard before clicking-and-dragging an Emacs window around somewhat underwhelming. It also looks a little goofy on UWS screens, so I'm disabling it for now.
