...
THe most useful Emacs command is ~execute-extended-command~. It should be painless to access from the home row.
...
#+begin_src emacs-lisp
  (ignore-errors (set-frame-font "JuliaMono-13"))
...
My journey through the various Emacs completion facilities has been long and twisty. I started with Helm, then spent several years using Ivy, and am now using Selectrum, with the consult and marginalia packages to yield an interface that is nicer and faster than Ivy.
...
#+begin_src emacs-lisp
  ...
  (use-package selectrum
    ...
    ;; Ido has a nice feature where it greedily deletes directories.
    ;; This is surprisingly nice and I was loathe to lose it with selectrum.
    ;; Pilfered from the vertico-directory extensions.
    ...
    (defun selectrum-directory-delete-char ()
      "Delete directory or char before point."
      (interactive)
      (unless (selectrum-directory-up)
        (call-interactively #'backward-delete-char)))

    (defun selectrum-directory-delete-word ()
    ...
    (:map selectrum-minibuffer-map
          ("DEL" . #'selectrum-directory-delete-char)
          ("M-DEL" . #'selectrum-directory-delete-word)
          ...
          ("<mouse-2>" . #'selectrum-select-current-candidate)
          ("C-c SPC" . #'selectrum-quick-select))
    :custom
...
After a long journey with ~company~, I've settled on just using the builtin completion-at-point facilities for autocomplete. The UI considerations afforded by Selectrum make it even nicer than what Company offered, and consistently faster, too. Someday I want to look into a more aggressive inline autocompletion thing like VSC
