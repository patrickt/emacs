...
#+begin_src emacs-lisp
  (use-package project
    ...
    :config
    ;; For whatever reason, .project.el gets confused about this file itself.
    (defun pt/override-project-root-detector (dir)
        (when (s-suffix? "emacs/readme.org" (buffer-file-name))
          (cons 'vc (file-name-directory (buffer-file-name)))))

    (add-hook 'project-find-functions #'pt/override-project-root-detector)

    :custom
    (project-switch-commands '((project-find-file "Find file") (magit-project-status "Magit" ?g)))
...
Built-in ~xref~ is powerful.

#+begin_src emacs-lisp
  (bind-key "s-r" #'xref-find-references)
  (bind-key "s-d" #'eldoc)
#+end_src
