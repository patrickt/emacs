...
I'll say it: modern Emacs Lisp is unfairly maligned. However, it was pretty fairly maligned before Emacs 24, which introduced lexical scoping.
...
#+begin_src emacs-lisp
  (use-package doom-themes
    :config
    (let ((chosen-theme 'doom-homage-black))
...
I'm trying out native tab-bar stuff. It's looking better.
...
Navigating forward and back through the history of recently visited files is pretty essential when jumping around a project with LSP.
...
#+begin_src emacs-lisp
  (use-package history
    ...
    :config
    (history-mode)
    (advice-add #'lsp-find-definition-mouse :before #'history-add-history)

    :bind (("s-[" . #'history-prev-history)
...
#+begin_src emacs-lisp
  ...
  (bind-key "C-(" #'centaur-tabs-backward)
  (bind-key "C-)" #'centaur-tabs-forward)
#+end_src
